...
static DBL HeightScale;             /* Fudge factors for SVGA scaling */
static DBL WidthScale;

/* Dithering error arrays should be dynamically allocated
   based on ImageWidth */
static signed short r_err[2][2050];   /* Allow up to 2048x??? resolutions. */
static signed short g_err[2][2050];
static signed short b_err[2][2050];

int In_Graph_Mode;                    /* Graphics mode supresses text display */
static int Dithered;
/************************
...
************************/
void MSDOS_Display_Init(width, height) 
  int width, height;
  {
   RMREGS inr, outr;
   unsigned long u;
   ...
   else
   {
      ...
      VideoMem = PM_mapPhysicalAddr(0xA0000L,0xFFFF);
...
************************/
static int Paradise_Chkbank(bank0, bank1)
...
void MSDOS_Display_Close()
   ...
   if (In_Graph_Mode)
      ...
      /*GNU*/
      if (bFIFTYLINES)
         spawnlp (P_WAIT, "mode", "mode", "co80,50", NULL);
...
************************/
void MSDOS_Display_Plot (x, y, Red, Green, Blue)
  int x, y;
...
************************/
static void hsv_to_rgb(hue, s, v, r, g, b)
...
*   
************************/
static void rgb_to_hsv(r, g, b, h, s, v)
   unsigned r, g, b;
   DBL *h, *s, *v;
   {
...
************************/
void MSDOS_Display_Plot_Rect  (x1, y1, x2, y2, Red, Green, Blue)
int x1, y1, x2, y2;
unsigned char Red, Green, Blue;
{
   int i,j, d;
...
int SelectVBEMode(int width,int height)
/****************************************************************************
*
...
Debug_Info("VBE_setBankA\n");
       ...
       VideoMem = VBE_getBankedPointer(&modeInfo);
    }
