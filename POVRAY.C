...
#include <ctype.h>
#include <time.h>     /* BP  */
#include <GnuType.h>  /* GNU */
#include <direct.h>   /* GNU */
...
MAIN_RETURN_TYPE main(int argc, PSZ *argv) /* GNU */
{
//register int i;
  int Diff_Frame;
  DBL Diff_Clock;
...
//  for (i = 1 ; i < argc ; i++ )       /*GNU*/
//  {
//    parse_option_line(argv[i]);
//  }

   ParseInit (argc, argv);              /*GNU*/
   ParseOptions ();
...
static void FrameRender()
{
  ...
  if (opts.Use_Slabs && bOneTime)/*GNU*/
  {
    Status_Info("Creating bounding slabs.");
  }
...
static void init_vars()
{
  ...
  Frame.Screen_Height = 480;
  Frame.Screen_Width  = 640;
  Root_Object = NULL;
  free_istack = NULL;
